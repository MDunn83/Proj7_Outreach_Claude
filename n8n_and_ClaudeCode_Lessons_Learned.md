...
- The correct sequence is: **LLM Chain → Gmail → Log → Update Last Contacted**.
  - Gmail fires first to confirm delivery.
  - The Log node receives output from Gmail (even though it carries no useful data — use cross-node refs to pull the email body from the LLM Chain node).
...
- In any node that runs after a `chainLlm` node, use cross-node references to retrieve data from earlier nodes:
  ...
  $('LLM Chain Ticket').item.json.text
  ```
